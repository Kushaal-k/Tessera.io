import { Activity, BrainCircuit, Cloud, Factory } from 'lucide-react';
...
export const DashboardOverview = ({ onNavigate }: { onNavigate: (view: string) => void }) => {
  ...
  return (
    <div className="flex-1 flex flex-col p-6 h-full relative z-10 overflow-y-auto custom-scrollbar">
      <div className="mb-6 p-6 rounded-2xl glass-panel bg-gradient-to-r from-sky-900/40 to-indigo-900/40 border border-sky-800/80 relative overflow-hidden">
        <div className="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-sky-500/20 rounded-full blur-3xl"></div>
        <div className="relative z-10">
          <h1 className="text-2xl font-bold text-white mb-2">Tessera Autonomous Engineering Cloud (75 Phases)</h1>
          <p className="text-slate-300 max-w-2xl">
            You have successfully reached the ultimate scale: a 75-Phase ecosystem. The platform now operates as a fully autonomous software engineering mesh.
          </p>
          <div className="mt-4 flex gap-3">
            <button onClick={() => onNavigate('TesseraCloudHub')} className="btn-primary"><Cloud size={16} /> Enter Cloud Hub</button>
            <button onClick={() => onNavigate('SoftwareFactoryControl')} className="btn-secondary"><Factory size={16} /> View Factory Mesh</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 grid grid-cols-2 gap-4">
          <div className="glass-panel p-5 border border-slate-800/80 rounded-xl bg-slate-950/50 flex flex-col justify-between cursor-pointer hover:border-emerald-500/50" onClick={() => onNavigate('TesseraCloudHub')}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-slate-400 text-xs font-medium">Platform Scale</p>
                <h3 className="text-2xl font-bold text-slate-100 mt-1">75 Phases</h3>
              </div>
              <div className="p-2 rounded-lg bg-emerald-500/20 text-emerald-400"><Activity size={20} /></div>
            </div>
          </div>
          <div className="glass-panel p-5 border border-slate-800/80 rounded-xl bg-slate-950/50 flex flex-col justify-between cursor-pointer hover:border-purple-500/50" onClick={() => onNavigate('CommandCenter')}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-slate-400 text-xs font-medium">AI Orchestration</p>
                <h3 className="text-2xl font-bold text-slate-100 mt-1">Global Mesh</h3>
              </div>
        ...
        <div className="glass-panel border border-slate-800/80 rounded-xl bg-slate-950/50 flex flex-col h-96">
          <div className="p-4 border-b border-slate-800 flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-slate-500">Full 1-75 Roadmap</p>
            </div>
