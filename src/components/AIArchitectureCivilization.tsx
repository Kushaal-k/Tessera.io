import { Hexagon, Zap, ArrowRight, Settings } from 'lucide-react';

export const AIArchitectureCivilization = () => {
  return (
    <div className="flex-1 flex flex-col p-6 h-full relative z-10 overflow-y-auto custom-scrollbar">
      {/* Header Banner */}
      <div className="mb-6 p-6 rounded-2xl glass-panel bg-slate-900/50 border border-slate-500/30 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4">
          <div className="p-3 rounded-xl bg-slate-950/50 border border-slate-800/80 shadow-lg">
            <Hexagon className="text-slate-400" size={28} />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-100">AI Architecture Civilization</h1>
            <p className="text-slate-400 text-sm mt-1">Create evolving software architectures.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="glass-panel border border-slate-800/80 rounded-xl bg-slate-950/50 p-6 flex-1 min-h-[300px] flex flex-col items-center justify-center text-center relative overflow-hidden group">
            ...
            <div className="w-16 h-16 rounded-full bg-slate-900 border border-slate-700 flex items-center justify-center mb-4 relative z-10">
              <Zap className="text-slate-400 opacity-70 animate-pulse" size={24} />
            </div>
        ...
        <div className="flex flex-col gap-4">
              ...
              {[1, 2, 3].map(i => (
                <div key={i} className="flex flex-col gap-1.5">
                  ...
                  <div className="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden border border-slate-800">
                    <div className={`bg-gradient-to-r from-slate-700 to-slate-500 h-1.5 rounded-full`} style={{ width: `${Math.random() * 60 + 40}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="glass-panel border border-slate-800/80 rounded-xl bg-slate-950/50 p-5 flex-1 flex flex-col justify-center items-center text-center">
             ...
             <p className="text-xs text-slate-400 font-mono">Phase 108 Online</p>
          </div>
        </div>
      </div>
    </div>
